...
def devuelve_tipo_dato(dato, variable):
  if dato.dtype == 'int64':
    dato = 'entero'
  elif dato.dtype == 'float':
    dato = 'flotante'
  elif dato.dtype == 'bool':
    dato = 'booleano'
  else:
    dato = 'objeto'  

  print(f'La variable {variable} es un dato {dato}.')

  return 


def cantidad_nulos(dato, variable):
  nulos = dato.isnull().sum()
  porcentaje = round(nulos / len(dato) * 100, 2)
  
  print(f'La variable {variable} tiene {nulos} datos nulos registrados que representan el {porcentaje} % del total de datos.')
